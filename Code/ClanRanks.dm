...
mob/verb/OpenClanRanks()
	set hidden=1
	var/HTML="<body bgcolor=[rgb(0,0,64)]><center><table bgcolor=[rgb(151,210,209)] border=1 width=100%>"
	HTML+="<tr><td colspan=100%><center><b><u>Ranks</u> <a href='?src=\ref[src];action=Create Rank;clan=\ref[src.Clan]'>\[Criar Rank\]</a>"
	for(var/datum/ClanRank/C in src.Clan.Ranks)
		HTML+="<tr><td><b>[C.name]<td><a href='?src=\ref[src];action=EditRank;Rank=\ref[C];clan=\ref[src.Clan]'>Editar Rank</a><td><a href='?src=\ref[src];action=DeleteRank;Rank=\ref[C];clan=\ref[src.Clan]'>Deletar Rank</a>"
...
mob/verb
	RenameClanRank()
		...
		src.EditClanRank.name=html_encode(copytext(input("Escreva o novo nome do Rank:","Rename Clan Rank",src.EditClanRank.name) as text,1,25))
		if(src.Clan!=ThisClan || src.EditClanRank!=ThisRank)	return
		if(src.EditClanRank.name=="Líder")	src.EditClanRank.name="--"
		for(var/datum/ClanRank/C in src.Clan.Ranks-src.EditClanRank)	if(C.name==src.EditClanRank.name)	src.EditClanRank.name="[src.EditClanRank.name]"
	...
	ChangeClanRankLevel()
		set hidden=1
		src.EditClanRank.Level=input("Coloque um nível para o Rank : 1 à 99","Rank Level",src.EditClanRank.Level) as num
	...
	ToggleRankAccess(var/Power2Toggle as null|text)
		...
		if(Power2Toggle in src.ClanRank.Powers)
			...
			alert("Não pode modificar os poderes que você não tem...","Nope")
			src.UpdateClanRankWindow()
