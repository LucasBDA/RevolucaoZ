...
mob/proc/SprintCheck(var/TapDir)
	...
	if(TapDir in src.SprintDirs)
		if(!src.Sprinting)
			...
			src.MovementSpeed=1
			if(!src.density)
				if(src.icon_state!="Beam" && src.icon_state!="none" && src.icon!='Candy.dmi')
					if(src.icon=='GokuKaioken.dmi')
						PlaySound(view(),'KaiokenAttack.ogg')
					else	PlaySound(view(),pick('ChainSwoop.ogg','Swoop.ogg','Dash.ogg'))
			if(!src.icon_state || src.icon_state=="Sprint" || src.icon_state=="fly")	src.ResetIS()
			if(src.icon_state=="fly" || src.icon=='Candy.dmi')			src.MovementSpeed=0.1
	else
...
mob/proc/GetMovementSpeed()
	...
	if(src.GravityTraining==1)	MovementDelay*=2
...
mob/Move(var/turf/NewLoc,NewDir)
	if(src.Sprinting && !src.density && NewLoc.Enter(src) && src.icon_state!="none")
		src.AddAura("Fly")
	...
	if(usr.icon_state=="Dragon")
		usr.layer=10
		if(usr.dir==1)	ShowEffect(usr.loc,"DragonFist",0,-16,usr.dir,1,0)
		if(usr.dir==2)	ShowEffect(usr.loc,"DragonFist",0,-16,usr.dir,1,0)
		if(usr.dir==4)	ShowEffect(usr.loc,"DragonFist",16,0,usr.dir,1,0)
		if(usr.dir==8)	ShowEffect(usr.loc,"DragonFist",-16,0,usr.dir,1,0)
	return ..()
