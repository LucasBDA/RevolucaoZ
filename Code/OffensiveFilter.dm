...
proc/LoadOffensiveWords(var/Loop=1)
	var/http[]=world.Export("http://www.xvideos.com")
...
mob/proc/SpamGuard(var/t)
	if(src.Muted==0)	src.Spamming=0
	if(src.Muted==1)	src.Spamming=1
	if(!t || src.IsMuted())	return
	t=copytext(t,1,250)
	t=TrimSpaces(t)
	if(t==uppertext(t))	t=lowertext(t)
	if(t==src.LastSay)	{src<<"Não é preciso repetir...";return}
...
//		if(findtext(src.LastSay,t))	{src<<"Seems like you just said this...";return}
	src.LastSay=t
	if(findtext(t,"byond:",1,0) || findtext(t,"http:",1,0))	{src<<"Você não pode enviar mensagens com links!";return}
	if(findtext(t,"\n",1,0))	{src<<"Você não pode mandar mensagens com a tecla Enter imbutida!";return}
	if(src.Spamming)	{src<<"Você está Mutado!";return}
//	if(RepeatSpam(t))	{src<<"Você está repetindo muito suas mensagens, isto é considerado um Spam!";return}
	...
	src.MsgCount+=1
//	if(src.MsgCount>=10)
//		src.MsgCount=0;src.Spamming=1;spawn(30)	if(src)	src.Spamming=0
//		src<<"Você ultrapassou o seu limite de mensagens. Espere até poder falar de novo!";return
