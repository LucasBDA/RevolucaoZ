mob/Player/verb/Friend()
	set src in world
	set category=null
	if(usr==src)	{usr<<"Você não pode adicionar você mesmo como amigo....";return}
	if(src.key in usr.Friends)	{usr<<"[src] já é seu amigo..";return}
	usr.Friends+=src.key
	usr.OnlineFriends+=src
	usr<<"<b><font color=green>[src] foi adicionado como amigo!"
	src<<"<b><font color=green>[usr] te adicionou como amigo!"
...
mob/verb
	...
	View_Forums()
		set hidden=1
		src<<link("http://www.byond.com/games/Shawn_EX/DBZRevolutionBrasil")
	...
	BAR()
		set hidden=1
		set category="Parceiros"
		usr<<link("http://www.byond.com/games/maxjason/bleacharrancarrevenger")
...
mob/verb/AbrirChat()
	set hidden=1
	winset(usr,"window2.Output2","is-visible=false;is-maximized=false;can-resize=false;titlebar=false")
	winset(usr,"window2.Output2","is-visible=true;is-maximized=false;can-resize=true;titlebar=true")*/
mob/proc/SendChat(var/t,var/Channel="GlobalChatPane")
	...
	var/FullMsg="<font color=[rgb(179,170,0)] size=1> [src.Title]</font size><b><font color=[src.NameColor]><a href='?src=\ref[src];action=ChatClick'>\icon[src]</a>[src]:[SubFont] [t]"
	if(Channel=="ChatPaneClan")
		...
		for(var/mob/M in Players)	M<<output("[ClanTag][FullMsg]","window2.output1") //CHAT DO JOGO (GLOBAL) chat global
