...
var/list/PowerUpModes=list("Ambos","Ki, somente","PL, somente","Ki, depois PL","PL, depois Ki","Nada")
mob/verb
	Volume()
	...
	CloseFullMap()
		...
		winset(src,"MainWindow.InfoPane","is-default=true;is-visible=true")
		winset(src,"MainWindow.ChatPanes","is-default=true;is-visible=true")
		winset(src,"FreeScaleWindow.InfoPane","is-default=false;is-visible=false")
		winset(src,"FreeScaleWindow.ChatPanes","is-default=false;is-visible=false")
	...
	BigIcons()
		set hidden=1
		winset(src,"MainWindow.MainMap","icon-size=41")
	NullTarget()
		set hidden=1
		src.TargetMob(null)
	ScreenShot()
	...
	ToggleTransparency()
		...
		if(!src.see_invisible)	src<<"Transparência Desligada"
		else	src<<"Transparência Ligada"
