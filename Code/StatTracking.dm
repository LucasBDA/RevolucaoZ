			...
			if(TrackingType=="List")
				...
				if(Listing=="Once")	if(!(Value in L[Stat]))
					...
					if(v=="RecordedTracked")
						if(Stat=="Days Played" && src.Level>1)	src.AddExp(100*100,"Recompensa do Dia")
						if(Stat=="Days Subscribed" && src.Level>1)	src.AddExp(100*1000,"Recompensa do Dia")
				else	L[Stat]+=Value
			else
				if(isnum(Value))
...
mob/verb/Sense()
	...
	var/Fmt="<font color=[rgb(255,255,255)]>"
	...
	for(var/v in src.RecordedTracked)
		...
		for(var/s in L)
			...
			else	if(EmLista(L[s]))	{var/list/LenList=L[s];L[s]="[LenList.len]"}
