...
obj/Blasts
	...
	New(var/mob/M,var/D,var/IS)
		...
		for(var/mob/Player/P in oview(7,src))	if(P.HasPerk("Heightened Senses"))	{WalkSpeed*=2;break}
		if(src.icon_state=="HomingKB" && M.Target && (abs(M.x-M.Target.x<=12) || abs(M.y-M.Target.y<=7)))	walk_towards(src,M.Target,WalkSpeed)
		else	walk(src,src.dir,WalkSpeed)
		if(src.icon_state=="HomingKB")	spawn(100)	src.loc=null
	Move()
		if(src.overlays==global.Freeze)	return
	...
	Bump(var/atom/A)
		...
		if(ismob(A) && src.Owner)
			...
			if(src.icon_state=="HealingKB")
				...
				if(src.Owner.ControlClients && M.HasPerk("A Prova de Balas") && M.icon_state!="koed" && M.PL>0)	{walk(src,BackAngleDir(src.dir));return}
				M.dir=get_dir(M,src)
						...
						if(chan==1)
							if(M.PLGeral<100000)
								...
								if(M.PLGeral>=100000 && M.PLGeral<1000000)
									M.MaxKi+=M.MaxKi*0.01
								else
									if(M.PLGeral>=1000000 && M.PLGeral<100000000)
										M.MaxKi+=M.MaxKi*0.001
									else	M.MaxKi+=M.MaxKi*0.0001
				...
				if(M.CanBeHit())
					if(M.icon_state=="Guard")
						M.dir=get_dir(M,src)
						if(src.icon_state=="GuardBreakKB")
							if(M.icon_state=="Guard")
								if(M.key!="Hkl" && M.key!="RLKS" && M.key!="Eventer" && M.key!="Rafafa47" && M.key!="DraguunnoNeko")
									M.Blocked("Guard Break KB",src.Owner,20)
						else
							if(M.key!="Hkl" && M.key!="RLKS" && M.key!="Eventer" && M.key!="Rafafa47" && M.key!="DraguunnoNeko")
		...
		PlaySound(view(),null,channel=55)
		PlaySound(view(),'explosion2.ogg',channel=55)
