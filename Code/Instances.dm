...
mob/proc/CanInstance()
	if(src.CurrentCP && src.CurrentMission)	{src<<"Não pode se deslocar assim durante missões!";return}
	if(src.InTournament)	{src<<"Não pode se deslocar assim durante Torneios!";return}
	if(src.Training || !src.CanAct())	{src<<"Não pode se deslocar assim por enquanto...";return}
	return 1
...
mob/proc/ChangeInstance(var/InstanceID)
	...
	var/obj/TurfType/Instances/Choice=input("Select Instance:","Zoning","Cancel") as null|anything in ChoiceList
