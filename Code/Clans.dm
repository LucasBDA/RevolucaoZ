...
mob/verb/ClearClanLog()
	if(src.HasClanRankPower("Clear Log"))
		...
		alert(src,"Você não tem permissão para limpar o Log do Clan!","Acesso negado!")
...
mob/verb/ChangedClanTab()
	...
	if(src.Clan)
		...
		src:Manage_Members()
		src:Clan_Management()
		src<<browse("<body bgcolor=[rgb(0,0,64)]><center><table border=1 width=100% bgcolor=[rgb(255,255,255)]>[src.Clan.ClanLog]","window=ClanLogPane.ClanLogBrowser")

mob/proc/ClanExpPane()
	var/HTML="<body bgcolor=[rgb(0,0,64)]><center><table border=1 width=100% bgcolor=[rgb(151,210,209)]>"
...
obj/HUD
	Clan_Management
		...
		screen_loc="12,1"
		desc="Ver opções, membros, Ranks, ou detalhes de seu Clan."
		MouseEntered()
			return ..()
...
mob/proc/InviteEveryone()
	...
	for(var/mob/M in PossibleInvites)
		...
		Counter+=1
		M.PopupHUD(new/obj/HUD/Popups/Clan_Invite(src,ThisGroup),"Convite de Clan de [src] para entrar no [ThisGroup.name]")
	...
	Invite_Members()
		...
		for(var/mob/M in Players)
			if(!M.Clan && !(ThisGroup.name in M.ClanInviteIgnores) && !("*Ignore All*" in M.ClanInviteIgnores))
				...
				PossibleInvites+=M
				CONTINUE
		var/mob/M=input("Convide um jogador para entrar no seu Clan:","Invite Clan Members") as null|anything in PossibleInvites
...
mob/proc/ClanInvite(var/mob/Inviter,var/datum/Clan/ThisGroup)
	...
	var/Choice=alert(src,"[Inviter] convidou você para entrar no Clan dele '[ThisGroup.name]'","Clan Invite","Join","Decline","Ignore")
	if(!ThisGroup || src.Clan)	{Inviter<<"[src] could not join your Clan";return}
	if(Choice=="Join")
		Inviter<<"[src] entrou no seu Clan"
		src.Clan=ThisGroup
		...
		SaveClans()
	else	Inviter<<"[src] recusou o seu convite."
...
mob/Clan/verb
	Online_Members()
		return ..()
		var/HTML="<body bgcolor=[rgb(0,0,64)]><center><table bgcolor=[rgb(255,255,255)] border=1 width=100%>"
	...
	Manage_Members()
		...
		var/TF="<td><center><b>"
		var/HTML="<body bgcolor=[rgb(0,0,64)]><center><table bgcolor=[rgb(255,255,255)] border=1 width=100%>"
		HTML+="<tr><td colspan=100%><center><b>~ [src.Clan.Members.len] Membros ~"
		HTML+="<tr>[TF]<u>Membro[TF]<u>Rank<td colspan=2><center><b><u>Opções<td><center><b><u>Última vez visto"
		var/SmallHTML=""
		for(var/v in src.Clan.Members)
			SmallHTML+="<tr>[TF][v][TF][src.Clan.Members[v]][TF]<a href='?src=\ref[src];action=Change Rank;member=[v];clan=\ref[src.Clan]'>Mudar Rank</a>[TF]<a href='?src=\ref[src];action=Kick Member;member=[v];clan=\ref[src.Clan]'>Chutar Membro</a>[TF][src.Clan.GetLastOnline(v)]"
...
mob/verb/View_Clans()
	...
	Text+="<center><table border=1 bgcolor=[rgb(151,210,209)] width=100%>"
	Text+="<tr><td colspan=100%><center>[Fmt][ClansShown] de [Clans.len] Clans estão sendo mostrados"
	Text+="<br>Apenas mostrando Clans com 10+ Membros"
	Text+="<tr><td><center>[Fmt]Clan<td><center>[Fmt]Líder<td><center>[Fmt]Membros<td><center>[Fmt]Online<td><center>[Fmt]Clan Exp adquiridos<td><center>[Fmt]Clan Exp gastos"
	Text+="[ClanText]"
...
mob/verb/Create_Clan()
	set hidden=1
	set category="Clan"
	if(usr.Level<500000) {PlaySound(src,'no.ogg');alert("Voce não tem Level suficiente para criar clans","Acesso negado!");return}
//	if(!src.Subscriber) {PlaySound(src,'no.ogg');alert("Criar Clans só podem ser permitidos em Eventos","Acesso negado!");return}
	if(src.Clan)	{alert("Você já está num Clan","Duplicate");return}
	var/datum/Clan/ThisClan=new
	ThisClan.Members+=src.key
	ThisClan.Members[src.key]="Líder"
	ThisClan.name=ClanNameGuard(input("Coloque o nome do seu Clan","Input Name","Clan do [src]") as text)
	if(!ThisClan.name)	{alert("Nome inválido!","Error");del ThisClan;return}
	...
	SaveClans()
	src.Clan=ThisClan
	src.SetClanRank("Líder")
	src.UpdateClanWindows()
	src.Clan.LogClanLog("[src] criou [src.Clan]")
	alert("Clan '[src.Clan.name]' foi criado!","Success!")
