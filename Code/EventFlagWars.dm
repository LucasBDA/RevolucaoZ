...
mob/proc/DropFlag(var/Reason="Got Too Heavy?")
	...
	global.ClanWarsEntrants<<"[FlagWarsTag]<b>[src.GetClanTag()][src] Derrubou uma bandeira <font color=[F.name]>[F.name] ! ([Reason])"
	F.ResetFlag()
...
proc/FlagsCaptured(var/obj/FlagWars/Flags/F1,var/obj/FlagWars/Flags/F2)
	...
	if(F2.CapturedBy)
		world<<"[FlagWarsTag] <b>[ClanTag][F1.CapturedBy] e [F2.CapturedBy] pontuaram com a bandeira <font color=[F1.name]>[F1.name] !"
		F2.CapturedBy.AddExp(500*100,"Flag Wars Capture")
		...
		world<<"[FlagWarsTag] <b>[ClanTag][F1.CapturedBy] pontuou com a bandeira <font color=[F1.name]>[F1.name] !"
		F1.CapturedBy.GiveMedal(new/obj/Medals/Flag_Bearer)
		F1.CapturedBy.Clan.AddClanExp(100,F1.CapturedBy)
	F1.CapturedBy.AddExp(500*100,"Flag Wars Capture")
...
obj/FlagWars
	Flags
		...
		verb/Capture()
			...
			global.ClanWarsEntrants<<"[FlagWarsTag] <b>[usr.GetClanTag()][usr] capturou uma bandeira <font color=[src.name]>[src.name] !"
		proc/ResetFlag()
			if(src.CapturedBy)
