...
mob/verb/SubscribeMenu()
	set hidden=1
	switch(alert("Seja um Subscribe!","Subscribe","Mais Informações","Checar Subscribe","Subscribe Agora"))
		if("Checar Subscribe")	usr.SubCheck()
...
/*		else
			...
			src<<"Current Date: [time2text(world.timeofday,"MM-DD-YY")]"
			src<<{"<a href="">Click here to Renew your Subscription!</a>"}
	else
		src<<"You are not Subscribed to Stray Games"
		src<<{"<a href="">Click here to Subscribe today!</a>"}
	src<<""*/
